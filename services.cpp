...
InvoiceData& findInvoice(Value recordId) {
	...
	if (SQL.Fetch()) {
		...
		RLOG("No invoice found for record id: " + AsString(recordId));
		NEVER();
	}
	...
	if (SQL.Fetch()) {
		...
		RLOG("SUM not successfull for record id: " + AsString(recordId));
		NEVER();
	}
...
InvoiceData& findBill(Value recordId) {
	...
	if (SQL.Fetch()) {
		...
		RLOG("No bill for record id: " + AsString(recordId));
		NEVER();
	}
...
InvoiceData& findRecord(Value recordId) {
	...
	if (SQL.Fetch()) {
		...
		RLOG("No record for id: " + AsString(recordId));
		NEVER();
	}
				
	// invoice items		
	SQL & Select(AMOUNT, NAME.Of(LOV_PRODUCT).As(COL_ITEM_NAME), 
		NAME.Of(LOV_UNIT).As(COL_UNIT_NAME))
	.From(RECORD_ITEM)
		.LeftJoin(LOV_PRODUCT).On(PROD_ID.Of(RECORD_ITEM) == ID.Of(LOV_PRODUCT))
		.LeftJoin(LOV_UNIT).On(UNIT_ID.Of(LOV_PRODUCT) == ID.Of(LOV_UNIT))
